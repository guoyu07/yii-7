...
class CDbCommandBuilder extends CComponent
{
	...
	public function createFindCommand($table,$criteria,$alias='t')
	{
		...
		$sql=($criteria->distinct ? 'SELECT DISTINCT':'SELECT')." {$select} FROM {$table->getRealTableName()} $alias";
	...
	public function createCountCommand($table,$criteria,$alias='t')
	{
		...
		else
		{
			...
			elseif($criteria->distinct)
			{
				...
				$sql="SELECT COUNT(*)";
			$sql.=" FROM {$table->getRealTableName()} $alias";
			$sql=$this->applyJoin($sql,$criteria->join);
			$sql=$this->applyCondition($sql,$criteria->condition);
		}
	...
	{
		$this->ensureTable($table);
		$sql="DELETE FROM {$table->getRealTableName()}";
		$sql=$this->applyJoin($sql,$criteria->join);
		$sql=$this->applyCondition($sql,$criteria->condition);
		$sql=$this->applyGroup($sql,$criteria->group);
		$sql=$this->applyHaving($sql,$criteria->having);
	...
	public function createInsertCommand($table,$data)
	{
		...
		$sql="INSERT INTO {$table->getRealTableName()} (".implode(', ',$fields).') VALUES ('.implode(', ',$placeholders).')';
		$command=$this->_connection->createCommand($sql);
	...
	public function createUpdateCommand($table,$data,$criteria)
	{
		...
		$sql="UPDATE {$table->getRealTableName()} SET ".implode(', ',$fields);
		$sql=$this->applyJoin($sql,$criteria->join);
		$sql=$this->applyCondition($sql,$criteria->condition);
		$sql=$this->applyOrder($sql,$criteria->order);
		$sql=$this->applyLimit($sql,$criteria->limit,$criteria->offset);
