...
class CDbTableSchema extends CComponent
{
	...
	/**
	 * this is add by duotin fm for sharding db table
	 * @var model reference to CActiveRecord
	 */
	public $model;
	...
	public function getRealTableName(){
		...
		if($this->model->getDbConnection()->tablePrefix!==null && strpos($tableName,'{{')!==false)
			 return preg_replace('/\{\{(.*?)\}\}/',$this->model->getDbConnection()->tablePrefix.'$1',$tableName);
		else
			return $tableName;
	}
