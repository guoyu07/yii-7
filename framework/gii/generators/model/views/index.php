...
<?php $form=$this->beginWidget('CCodeForm', array('model'=>$model)); ?>
	...
	<div class="row sticky">
		<?php echo $form->labelEx($model,'modNum'); ?>
		<?php echo $form->textField($model,'modNum', array('size'=>65)); ?>
		<div class="tooltip">
			如果是取摸类型的分表使用
		</div>
		<?php echo $form->error($model,'modNum'); ?>
	</div>
	<div class="row sticky">
		<?php echo $form->labelEx($model,'shardingKey'); ?>
		<?php echo $form->textField($model,'shardingKey', array('size'=>65)); ?>
		<div class="tooltip">
			分表的依据字段，如：对user_id%8 就填写user_id
		</div>
		<?php echo $form->error($model,'shardingKey'); ?>
	</div>
	<div class="row sticky">
		<?php echo $form->labelEx($model,'shardingType'); ?>
		<?php echo $form->textField($model,'shardingType', array('size'=>65)); ?>
		<div class="tooltip">
			分表类型，mod，date<code>mod</code>
		</div>
		<?php echo $form->error($model,'shardingType'); ?>
	</div>
	<div class="row sticky">
		<?php echo $form->labelEx($model,'idTableName'); ?>
		<?php echo $form->textField($model,'idTableName', array('size'=>65)); ?>
		<div class="tooltip">
			主键生成器的ID，对应的是duotin_sharding_table_unique_key表里面的tableName字段
		</div>
		<?php echo $form->error($model,'idTableName'); ?>
	</div>
